...
class Bot(Sprite):
    ...
    def move(self, window):
        ...
        if self.x < 0 or self.right > setting_win["WIDTH"] or self.y < 0 or self.bottom > setting_win["HEIGHT"] or self.collidelist(wall_list) != -1:
            self.SPEED *= -1
    ...
    def shoot(self, window, hero):
        ...
        if self.BULLET.x < 0 or self.BULLET.right > setting_win["WIDTH"] or self.BULLET.y < 0 or self.BULLET.y > setting_win["HEIGHT"]:
            self.BULLET.x = self.x
            self.BULLET.y = self.centery
        window.blit(bullet_image, (self.BULLET.x, self.BULLET.y))
